# VS Code Tutorial
## Author: Zachary Wartell, Ph.D.

<div><br>
License:
<a rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/"> <img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by-nc-sa/4.0/88x31.png"></a>
<br>
<br>
</div>

Instructions and interactive grading rubric for a VS Code IDE configuration and setup for Dr. Wartell's courses plus a gradedtutorial based on readings in [https://code.visualstudio.com/]
required in Dr. Wartell's course work and research projects.

- WWW: 
    - https://webpages.charlotte.edu/zwartell/Teaching/itcs-vs-code-tutorial
- GIT: https://github.com/ZacharyWartell/git-tutorial-and-coding-standards.git
